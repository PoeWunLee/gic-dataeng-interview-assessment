...
@pytest.mark.parametrize(
   "input_date_partition,input_fund_csv,input_is_repeat",
        [
            ("2022-01-31","Applebead.csv",True),
            ("2023-08-31","Belaware.csv",True),
            ("2022-11-30","Catalysm.csv",True),
            ("2023-02-28","Gohen.csv",True),
            ("2025-06-30","Leeder.csv",True),
            ("2024-05-31","Magnum.csv",True),
            ("2024-04-30","Trustmind.csv",True),
            ("2024-07-31","Virtous.csv",True),
            ("2021-10-30","Wallington.csv",True),
            ("2019-03-31","Whitestone.csv",True)
        ]
)
def test_idempotent_load(input_date_partition, input_fund_csv, input_is_repeat,make_staging,make_connection, tmp_path):
    #arrange - connection and staging files and directories
    csv_path = make_staging
    make_connection
    ...
    assert len(df_from_query)==len(df_from_csv)